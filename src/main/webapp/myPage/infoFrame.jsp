...
<c:choose>
	...
	<c:otherwise>
		...
		<c:forEach var="user" items="${userResult.rows}">
			<c:set var="nicknamePlaceholder" value="${empty user.nickname ? '닉네임이 없습니다.' : user.nickname}" />
			<c:set var="phonePlaceholder" value="${empty user.phone ? '전화번호가 없습니다.' : user.phone}" />
			<c:set var="emailPlaceholder" value="${empty user.email ? '이메일이 없습니다.' : user.email}" />

			<c:if test="${param.error eq 'phone_exists'}">
				<script>alert('이미 등록된 전화번호입니다.');</script>
			</c:if>

			<c:if test="${param.error eq 'email_exists'}">
				<script>alert('이미 등록된 이메일입니다.');</script>
			</c:if>
